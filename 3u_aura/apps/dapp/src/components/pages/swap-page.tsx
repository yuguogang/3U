...
// 页面标题组件
function PageTitle({ title, showDetail = true }: { title: string; showDetail?: boolean }) {
  return (
    <div className="relative w-full h-6 mb-6">
      ...
      {/* 明细链接 */}
      {showDetail && (
        <Link href="/swap/history" className="absolute right-[32px] top-0 text-sm font-semibold text-white text-right leading-6">
          明细
        </Link>
      )}
    </div>
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-2">
        <div className="flex items-center justify-between">
          <TokenSelector token={token} balance={balance} />
  ...
  return (
    <div className="flex justify-center py-2">
      <button 
        ...
        style={{
          background: "linear-gradient(135deg, rgba(56, 58, 70, 1) 2%, rgba(70, 77, 104, 0.33) 97%)",
        }}
      >
        <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
...
function SlippageButton() {
  return (
    <button className="absolute top-4 right-4 w-9 h-9 rounded-full flex items-center justify-center bg-white/10 hover:bg-white/20 transition-colors">
...
export function SwapPage() {
  const [fromValue, setFromValue] = useState("500.00");
  const [toValue, setToValue] = useState("500.00");

  const handleSwap = () => {
    // 交换逻辑
    console.log("Swap:", fromValue, "->", toValue);
  };
    ...
    <MobileLayout showNav={true} showBottomNav={true}>
      <div className="px-4 pt-4">
        ...
        <div className="flex justify-center mt-8">
          <GradientButton onClick={handleSwap} className="w-[173px]">
            开始兑换
          </GradientButton>
        </div>
      </div>
